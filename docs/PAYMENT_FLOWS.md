...
- Machine-readable inputs are preferred: `standardised_code`/`standardized_code`, `category`, `unified_code`, connector and issuer codes, `error_code`, `issuer_error_code`, `status`, and `cancellation_reason`. The backend also inspects documented unified `message`, `description`, and `user_guidance_message`; connector `message` and `reason`; issuer `message`; and `error_message`, but only through normalized exact-value allowlists. It does not use substring/fuzzy matching or expose those strings. This narrow text fallback exists for documented or observed Dummy connector labels when no discriminating machine code is available.
...
- A terminal failed status never changes campaign metrics and offers safe, reason-specific guidance plus a direct route to restart checkout. A failed/cancelled latest attempt with a missing or `unknown` reason is atomically claimed for one `force_sync` enrichment retrieve. `failure_enrichment_attempted_at` prevents repeated terminal polling; a meaningful terminal reason is never retrieved merely for UI enrichment.
...
When hosted sandbox behavior needs inspection, set the Edge Function secret `HYPERSWITCH_FAILURE_DIAGNOSTICS=true` while `HYPERSWITCH_BASE_URL` is exactly the hosted sandbox origin, deploy the payment functions, and submit the four official Dummy scenarios through the Hyperswitch Payment Element: card declined, insufficient funds, lost card, and stolen card. Trigger or wait for `payment-status`, then inspect Edge Function logs for `Hyperswitch sandbox failure diagnostic`.
