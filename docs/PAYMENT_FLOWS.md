...
The `google-pay-diagnostic` Edge Function and backend-only `google_pay_diagnostic_events` table are temporary instrumentation for investigating Google Pay `OR_BIBED_06`. The browser observer records only an allowlisted error summary and safe payment-request configuration; the Edge Function authenticates the donation status capability, sanitizes the payload again, and caps storage at ten events per donation. It does not change payment creation, confirmation, redirects, status reconciliation, or webhooks. Remove the observer, function, configuration, table, and this note when the investigation is complete.
