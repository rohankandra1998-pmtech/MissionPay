...
`payment_attempts.failure_enrichment_attempted_at` is the completed-at marker for terminal UI recovery, while `failure_enrichment_claimed_at` is a short-lived concurrency lease. When a webhook leaves the latest failed/cancelled attempt `unknown`, `payment-status` claims the lease and performs one expanded, forced Hyperswitch retrieve. Success completes the marker; transient failure releases the lease; abandoned claims become retryable after the lease interval. A successful retrieve that remains unknown is not polled indefinitely.
...
- Fundraisers can read non-provider attempt columns only through campaigns they own. Provider failure snapshots and diagnostic columns have no client `SELECT` grant.
