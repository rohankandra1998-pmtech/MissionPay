...
import { ArrowRight, CheckCircle2, Clock3, Copy, RefreshCw, RotateCcw, XCircle } from "lucide-react";
...
interface StatusPayload extends Donation { campaign: { title: string; slug: string }; next_charge_at?: string; recurring_status?: string; recurring_payment_method_ready?: boolean; refund_url?: string; refund_request_status?: "pending" | "approved" | "declined"; refund_status?: "initiating" | "pending" | "review" | "succeeded" | "failed"; failure?: { reason: PaymentFailureReason } }
...
export function DonationStatusPage() {
  ...
  if (donation.status === "refunded") return <main className="status-page status-page--success"><RotateCcw /><p className="eyebrow">Refund complete</p><h1>Donation refunded</h1><p>Your {formatMoney(donation.amount_cents)} donation to <strong>{donation.campaign.title}</strong> has been refunded to the original payment method.</p><dl className="confirmation-card"><div><dt>Amount</dt><dd>{formatMoney(donation.amount_cents)}</dd></div><div><dt>Reference</dt><dd>{donation.id.slice(0, 8).toUpperCase()}</dd></div><div><dt>Status</dt><dd>Refunded</dd></div></dl>{donation.frequency === "monthly" && <div className="refund-monthly-note"><strong>Your monthly plan is unchanged.</strong><p>This refund applied only to this completed charge; future monthly donations remain scheduled separately.</p></div>}<div className="status-actions"><Link className="button button--dark" to={`/campaigns/${donation.campaign.slug}`}>Return to campaign</Link>{donation.refund_url && <a className="button button--outline" href={donation.refund_url}>View refund</a>}</div></main>;
