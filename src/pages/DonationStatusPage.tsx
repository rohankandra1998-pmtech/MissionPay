...
interface StatusPayload extends Donation { campaign: { title: string; slug: string }; next_charge_at?: string; recurring_status?: string }
...
export function DonationStatusPage() {
  ...
  if (donation.status !== "succeeded") return <main className="status-page"><XCircle /><p className="eyebrow">Payment not completed</p><h1>Your donation was not charged.</h1><p>The attempt was not added to the campaign total. You can safely try again.</p><div className="status-actions"><Link className="button button--coral" to={`/campaigns/${donation.campaign.slug}`}>Try another donation <ArrowRight size={17} /></Link><Link className="button button--outline" to={`/campaigns/${donation.campaign.slug}`}>Return to campaign</Link></div></main>;
