    ...
    <main className="checkout-page">
      <div className="checkout-shell">
        ...
        <div className="checkout-panel">
          ...
          {step === "payment" && session && <div><div className="payment-heading"><span><Check size={17} /></span><div><h2>Secure payment</h2><p>{formatMoney(cents)} {frequency === "monthly" ? "today, then monthly" : "one-time"}</p></div></div><HyperswitchCheckout clientSecret={session.client_secret} donationId={session.donation_id} statusToken={session.status_token} managementToken={session.recurring_management_token} frequency={frequency} /></div>}
        </div>
      </div>
    </main>
  );
