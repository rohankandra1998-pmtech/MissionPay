...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const paymentId = String(object.payment_id ?? object.id ?? "");
    ...
    const { data: inserted, error: insertError } = await admin.from("payment_events").insert({ provider_event_id: eventId, event_type: eventType, payment_attempt_id: attempt?.id ?? null, payload: event, provider_updated_at: object.updated ?? null }).select("id").maybeSingle();
    if (insertError?.code === "23505") return new Response("Already processed", { status: 200 });
    if (insertError) throw insertError;
    if (paymentId && attempt) await reconcilePayment(admin, { ...object, payment_id: paymentId });
