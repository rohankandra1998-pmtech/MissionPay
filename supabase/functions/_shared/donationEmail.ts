...
export function buildDonationConfirmationEmail(data: DonationConfirmationData): DonationConfirmationMessage {
  ...
  const monthlyStatus = data.frequency === "monthly"
    ? `<tr><td style="padding:7px 0;color:#687076">Monthly donation</td><td style="padding:7px 0;text-align:right;font-weight:600">${isActiveMonthly ? "Active" : "Confirmation pending plan activation"}</td></tr>`
    : "";
  ...
  const lines = [
    ...
    `Frequency: ${frequency}`,
    data.frequency === "monthly" ? `Monthly donation: ${isActiveMonthly ? "Active" : "Confirmation pending plan activation"}` : "",
    ...
    "Your payment was confirmed securely through MissionPay's payment flow.", `View campaign: ${data.campaignUrl}`,
  ].filter((line) => line !== "");
